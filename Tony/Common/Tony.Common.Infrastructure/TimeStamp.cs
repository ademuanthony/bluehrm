...
namespace Tony.Common.Infrastructure
{
    public class TimeStamp
    {
        ...
        {
            var now = new TimeStamp(true);
            var timeEllapsed = now.Date.Subtract(Date);
            if (timeEllapsed.TotalDays >= 1) return string.Format("{0} days ago", timeEllapsed.TotalDays);
            if (timeEllapsed.TotalHours >= 1) return string.Format("{0} hours ago", timeEllapsed.TotalHours);
            if (timeEllapsed.TotalMinutes >= 1) return string.Format("{0} days ago", timeEllapsed.TotalMinutes);
            return timeEllapsed.TotalSeconds >= 1 ? string.Format("{0} days ago", timeEllapsed.TotalSeconds) : "Just Now";
        }
        /// <summary>
    ...
    public static class TimeStampExtenssion
    {
        public static string ToTimeAgo(this TimeStamp timeStamp)
        {
            return timeStamp.ToTimeAgo();
        }
        public static string ToTimeAgo(this DateTime dateTime)
        {
            return new TimeStamp(dateTime).ToTimeAgo();
        }
