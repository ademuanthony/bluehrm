...
namespace Tony.Common.Infrastructure
{
    public class NumberToEnglish
    {
        ...
        {
            if (month < 0 || month > 12)
                throw new ArgumentOutOfRangeException("month", "Invalid month representation");
            return monthMap[month];
        }

        public static string GetNumbersPosition(int number)
        {
            if (number >= 10 && number <= 20)
            {
                return number + "th";
            }
            var lastDigit = int.Parse(number.ToString().Substring(number.ToString().Length - 1, 1));
            switch (lastDigit)
            {
                case 1:
                    return number + "st";
    ...
    public static class NumberExtessions
    {
        public static string ToNumberPosition(this int number)
        {
            return NumberToEnglish.GetNumbersPosition(number);
        }
    }
}
