...
namespace SoftBreeze.Tests
{
    ...
    /// Provides methods to easily work with <see cref="BlueHrmDbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase
    {
        ...
        {
            //Seed initial data
            UsingDbContext(context => new InitialDbBuilder(context).Create());
        ...
        protected override void AddModules(ITypeList<AbpModule> modules)
        {
            ...
            modules.Add<BlueHrmTestModule>();
        }

        #region UsingDbContext

        protected void UsingDbContext(Action<BlueHrmDbContext> action)
        {
            using (var context = LocalIocManager.Resolve<BlueHrmDbContext>())
            {
        ...
        protected async Task UsingDbContextAsync(Action<BlueHrmDbContext> action)
        {
            using (var context = LocalIocManager.Resolve<BlueHrmDbContext>())
            {
                context.DisableAllFilters();
        ...
        protected T UsingDbContext<T>(Func<BlueHrmDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<BlueHrmDbContext, Task<T>> func)
        {
            T result;
